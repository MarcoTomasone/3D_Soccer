...
export class MeshLoader {
	...
	static async LoadOBJAndMesh(gl, object) {
		if (debug && debug == true) console.log("Loading mesh " + object.name + " from " + object.filePath + (object.mtlPath ? " with MTL file " + object.mtlPath : ""));
		// Load OBJ file
		...
		if (!object.mtlPath) {
			...
			if (debug && debug == true) console.log("Loading manually defined MTL file " + object.mtlPath);
		...
		object.parts = obj.geometries.map(({ material, data }) => { // Since each geometry has it's own buffer, we have to load them separately
		...
		if (debug && debug == true) console.log("Loaded mesh for " + object.name + ". ", object);
	}
}
